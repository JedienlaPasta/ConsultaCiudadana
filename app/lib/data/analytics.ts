...
export async function getSurveyAnalytics(surveyId: number): Promise<SurveyAnalytics> {
  ...
  try {
    ...
    const participantsResult = await participantsRequest
    ...
    const totalParticipants = participantsResult.recordset[0]?.total_participants || 0;

    // 2. Get participation by date
    const participationByDateRequest = pool.request();
    const participationByDateResult = await participationByDateRequest
      .input("survey_id", sql.Int, surveyId)
      .query(`
        SELECT 
          CAST(created_at AS DATE) as participation_date,
    ...
    const participationByDate = participationByDateResult.recordset.map(row => ({
    ...
    const questionResultsResult = await questionResultsRequest
      .input("survey_id", sql.Int, surveyId)
      .query(`
        SELECT 
          p.id as question_id,
        ...
        GROUP BY p.id, p.question, o.id, o.option_name
    ...
    const questionResults = Array.from(questionResultsMap.values()).map(question => ({
      ...question,
      options: question.options.map(option => ({
        ...option,
        percentage: question.totalVotes > 0 ? (option.voteCount / question.totalVotes) * 100 : 0
      }))
    }));

    // 4. Get sector participation (for map questions)
    const sectorParticipationRequest = pool.request();
    const sectorParticipationResult = await sectorParticipationRequest
      .input("survey_id", sql.Int, surveyId)
      .query(`
        SELECT 
          o.sector,
    ...
    const sectorParticipation = sectorParticipationResult.recordset.map(row => ({
      sector: row.sector,
      participantCount: row.participant_count,
      percentage: totalParticipants > 0 ? (row.participant_count / totalParticipants) * 100 : 0
    }));
