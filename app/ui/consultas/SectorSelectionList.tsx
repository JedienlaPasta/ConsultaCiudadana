...
function SectorItem({ sector, isSelected, onSelect }: SectorItemProps) {
  return (
    <div
      ...
      className={`group md:p-4s relative flex cursor-pointer flex-col rounded-lg border px-4 py-3 transition-all duration-200 hover:border-blue-200 hover:shadow-md ${isSelected ? "!border-[#0F69C4] !bg-blue-50 shadow-md" : "border-gray-200"}`}
    >
      {isSelected && (
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        <div className="flex items-center gap-1">
          ...
          <span className="font-medium">{sector?.sector_population || ""}</span>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="font-medium">{sector?.sector_area || ""}</span>
        </div>
      </div>
    </div>
  );
